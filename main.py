"""
🚀 JARVIS OS – IRON MAN LEVEL REAL-TIME VOICE AI ASSISTANT

Complete state machine implementation with:
- 7 operational states (IDLE, LISTENING, WAKE, VERIFICATION, ACTIVE_LISTENING, PROCESSING, RESPONDING)
...
This is the main orchestrator for JARVIS.
"""
...
def run_jarvis():
    """
    🚀 PHASE 20: Main Jarvis OS loop with real-time voice AI system
    
    Features:
    - Input validation (3+ words, garbage filtering)
    - Interrupt handling (stop, jarvis, wait)
    - Priority system (INTERRUPT > COMMAND > QUESTION > IGNORE)
    - Smart AI trigger (only process questions)
    - 10-second AI timeout
    - Anti-loop system (prevent repeated responses)
    - Clean response mode (direct answers only)
    
    Flow:
    1. Listen for valid input (3+ words, not garbage)
    2. Check for interrupts while speaking
    3. Detect intent (command vs question)
    4. Execute with proper priority handling
    5. Wait for response with timeout
    6. Speak response (skip if duplicate)
    7. Loop
    """
    global state
    ...
    # Give startup message time to be spoken
    time.sleep(2)
    # Announce listening state (short prompt)
    speak("Listening")
    ...
    while True:
        try:
            # 🎤 GET INPUT (thread-safe)
            current_input = get_latest_input()

            lower_input = (current_input or "").lower()

            # Expire active session after timeout
            if active and (time.time() - last_active_time) > VERIFIED_TIMEOUT:
                print("⏳ Active session expired — require wake word again")
                active = False

            # Skip if no meaningful input or duplicate
            if not is_valid_input(current_input) or current_input == last_processed_text:
                time.sleep(0.1)
                continue

            # Require wake word to start a new session when not active
            if not active and "jarvis" not in (current_input or "").lower():
                # Ignore ambient speech unless it includes wake word
                time.sleep(0.05)
                continue

            # If we must require wake word for every command (no speaker verification),
            # ignore inputs that don't include the wake word to avoid ambient capture.
            if REQUIRE_WAKE_FOR_ALL and "jarvis" not in lower_input:
                time.sleep(0.05)
                continue

            # Strict mode: when verification isn't available, only accept commands
            # that explicitly contain the wake word even during an active session.
            # Allow interrupt keywords (stop/wait/etc.) to still interrupt.
            ...
            if get_is_speaking():
                ...
                if interrupt_by_wake or interrupt_by_active or interrupt_by_keyword:
                    ...
                    print("⏭️ Jarvis is speaking, ignoring input")
                    time.sleep(0.1)
                    continue

            # ════════════════════════════════════════════════════════
            # 👋 GREETING RESPONSE (Hello/Hi directed to Jarvis)
            # Only respond to greetings that include the wake word to avoid false positives
            # ════════════════════════════════════════════════════════
            lower_input = current_input.lower()
            if any(greeting in lower_input for greeting in ["hello", "hi", "hey"]) and "jarvis" in lower_input:
                # Treat greeting as wake-word activation; verify before activating
            ...
            if not ai_response:
                # Timeout: use the standard timeout message
                ai_response = "I am having trouble thinking right now."
                print("⏱️ PHASE 20: AI TIMEOUT - Using standard response")
            else:
                print(f"✅ AI responded: {ai_response[:100]}")
                
                # 🧹 PHASE 20: Clean response (remove unnecessary follow-ups)
            ...
            # Wait for response to be fully spoken before continuing
            # This prevents listener from picking up our own speech
            time.sleep(2)
